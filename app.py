...
try:
    ...
    st.error("Model or scaler file not found. Please ensure 'logistic_regression_model.pkl' and 'scaler.pkl' are in the same directory.")
    st.stop()
...
st.markdown(f"""
<style>
    /* Apply background to the entire app view container */
...
# --- Main Title ---
st.title("Cardiac Disease Prediction App ❤️")
...
def show_user_dashboard():
    ...
    with tabs[0]:
        ...
        input_data = pd.DataFrame({
            "Age": [age], "Sex": [sex_val], "ChestPainType": [chest_pain_val],
            "RestingBP": [resting_bp], "Cholesterol": [cholesterol], "FastingBS": [fasting_bs_val],
            "RestingECG": [resting_ecg_val], "MaxHR": [max_hr], "ExerciseAngina": [exercise_angina_val],
            "Oldpeak": [oldpeak], "ST_Slope": [slope_val]
        })

        # --- NEW CODE: SCALE THE SINGLE INPUT DATA ---
        input_data_scaled = scaler.transform(input_data)
        # --- END NEW CODE ---

        def predict_logistic_regression(data):
            try:
                # The model is now loaded at the beginning of the script
        ...
        if st.button("Predict", key="user_predict"):
            with st.spinner('Making prediction...'):
                time.sleep(1)
                # Pass the scaled data to the prediction function
                result = predict_logistic_regression(input_data_scaled)
                ...
                if missing_cols:
                    ...
                    if st.button("Predict Bulk Data"):
                        with st.spinner('Processing bulk predictions...'):
                            ...
                            # --- NEW CODE: SCALE THE BULK DATA ---
                            bulk_data_scaled = scaler.transform(bulk_data_imputed)
                            # --- END NEW CODE ---
                            ...
                            try:
                                ...
                                predictions = model.predict(bulk_data_scaled)
...
def show_login_page():
    """ Renders the login and signup page for both users and admins. """
    ...
    # Admin Signup (Hidden)
    st.markdown("---")
    st.subheader("Admin Signup (First Time Setup)")
